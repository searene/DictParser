...
var universal_1 = require("./../src/universal");
...
describe('Test private methods in DictionaryManager', function () {
    ...
    beforeEach(function () {
        // prepare database
        pathToDb = path.join(universal_1.Constant.rootPathOfModule, 'resources/test.db');
        ...
        dictTable = universal_1.Constant.dictTableName;
    });
    afterEach(function () {
        // clean test files
        os_1.FSHelper.removeFileIfExists(pathToDb);
...
describe('test unknown functions', function () {
    it('sqlite3', function () {
        ...
        return databaseManager.createDictTableIfNotExists()
            ...
            return new Promise(function (resolve, reject) {
                db.serialize(function () {
                    db.run("INSERT OR REPLACE INTO " + universal_1.Constant.dictTableName + "\n                            (DICT_ID, DICT_FILE, RESOURCE)\n                            VALUES (2, 'name', 'resource')", function (err) {
                        if (err != null) {
                            reject(err);
                        }
                    });
