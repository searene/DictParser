...
{the }sample headword
   ...
   [m2][*][ex][lang id=1033]to do smth. with in complete abandon[/lang] — делать что-л., совершенно забыв обо всем[/ex][/*][/m]`;
        it("Simple entry test", () => {
            let stateMachine = new DSLStateMachine(simpleEntryContents);
            let wordTree = stateMachine.run();
            wordTree.entry.forEach(entry => {
                logger.debug(entry);
            });
        });
        it("Medium entry test", () => {
            let stateMachine = new DSLStateMachine(mediumEntryContents);
            let wordTree = stateMachine.run();
            wordTree.entry.forEach(entry => {
                logger.debug(entry);
            });
        ...
        it("Complex entry test", () => {
            let stateMachine = new DSLStateMachine(complexEntryContents);
            let wordTree = stateMachine.run();
            wordTree.entry.forEach(entry => {
                logger.debug(entry);
            });
        ...
        it("Typical entry test", () => {
            let stateMachine = new DSLStateMachine(typicalEntryContents);
            let wordTree = stateMachine.run();
            wordTree.entry.forEach(entry => {
                logger.debug(entry);
            });
