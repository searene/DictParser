 ...
 * License: MIT
 *
 * Modified by Searene on 2017.02.08 to use with nodejs
 */
var fs = require('fs');
(function (GLOBAL) {
    ...
    function zero_terminated_string(buffer, offset) {
        ...
        for (var n = 1; true; n++) {
            ...
            var view = new Uint8Array(buffer.slice(offset, offset + n * 1024)), end = Array.prototype.indexOf.call(view, 0);
            if (end == -1) {
                end = view.length;
                if (end == 0)
    ...
    function read_gzip_header(buffer) {
        var FTEXT = 1, FHCRC = 2, FEXTRA = 4, FNAME = 8, FCOMMENT = 16;
        var position = 0, view = new Uint8Array(buffer, position, 10), header_data = {
            "ID1": 0,
            "ID2": 0,
            "CM": 0,
            "FLG": 0,
    ...
    var DictZipFile = (function () {
        var cls = function (f, gunzip_func) {
            var dzfile = f, gunzip = gunzip_func, verified = false, gzip_header, ver, chlen = 0, chcnt = 0, chunks = [], fileContents;
            if (!(gunzip instanceof Function)) {
                throw new Error("Given gunzip_func is not a function.");
            }
            this.get_chunks = function (buffer) {
                gzip_header = read_gzip_header(buffer);
                var subfields = gzip_header["FEXTRA"]["SUBFIELDS"], found = false, sf;
                ...
                if (!found) {
                    ...
                    for (var i = 0, chpos = 0; i < chcnt && 2 * i + 6 < b.length; i++) {
                        // length of the i-th chunck after dictzip
            ...
            this.load = function () {
                var get_chunks = this.get_chunks;
                return new Promise(function (resolve, reject) {
                    fs.readFile(dzfile, function (err, data) {
                        fileContents = data.buffer;
                        get_chunks(data.buffer);
                        resolve();
                    });
                });
            };
            this.read = function (pos, len) {
                return new Promise(function (resolve, reject) {
                    ...
                    offset = pos - firstchunk * chlen, finish = offset + len, out_buffer = new ArrayBuffer(0), in_buffer = fileContents.slice(gzip_header["LENGTH"] + chunks[firstchunk][0], gzip_header["LENGTH"] + chunks[lastchunk][0] + chunks[lastchunk][1]);
                    for (var i = firstchunk, j = 0; i <= lastchunk && j < in_buffer.byteLength; j += chunks[i][1], i++) {
                        var chunk = in_buffer.slice(j, j + chunks[i][1]), inflated = gunzip(chunk, 0, chunk.byteLength);
...
//# sourceMappingURL=dictzip.js.map