...
export class DSLDictionary extends Dictionary {
    ...
    protected _dictionaryName: string = 'dsl';
    ...
    private async getEncoding(fileContents: Buffer): Promise<string> {
        let encoding = await DetectEncoding.detectEncodingByBOM(fileContents);
        return encoding.isDetectionSuccessful ? encoding.encoding : 'utf8';
    }
    ...
    private async getDictContents(dictFile: string): Promise<string> {
        ...
        let encoding = await this.getEncoding(dictBinaryContents);
        let dictContents: string = dictBinaryContents.toString(encoding);
        return dictContents;
    }
